...
export const INTENTION_CONFIG: Record<IntentionType, IntentionConfigEntry> = {
  trusted: {
    label: 'Trusted',
    color: INTENTION_HEX.trusted,
    gradientEnd: '#4ADE80',
    cssClass: 'trusted',
    intentionPurpose: null,
    predicateLabel: 'trusts',
  },
  distrusted: {
    label: 'Distrusted',
    color: INTENTION_HEX.distrusted,
    gradientEnd: '#F87171',
    cssClass: 'distrusted',
    intentionPurpose: null,
    predicateLabel: 'distrusts',
  },
  work: {
    label: 'Work',
    color: INTENTION_HEX.work,
    gradientEnd: '#60A5FA',
    cssClass: 'work',
    intentionPurpose: 'for_work',
    predicateLabel: 'visits for work',
  },
  learning: {
    label: 'Learning',
    color: INTENTION_HEX.learning,
    gradientEnd: '#22D3EE',
    cssClass: 'learning',
    intentionPurpose: 'for_learning',
    predicateLabel: 'visits for learning',
  },
  fun: {
    label: 'Fun',
    color: INTENTION_HEX.fun,
    gradientEnd: '#FBBF24',
    cssClass: 'fun',
    intentionPurpose: 'for_fun',
    predicateLabel: 'visits for fun',
  },
  inspiration: {
    label: 'Inspiration',
    color: INTENTION_HEX.inspiration,
    gradientEnd: '#A78BFA',
    cssClass: 'inspiration',
    intentionPurpose: 'for_inspiration',
    predicateLabel: 'visits for inspiration',
  },
  buying: {
    label: 'Buying',
    color: INTENTION_HEX.buying,
    gradientEnd: '#F472B6',
    cssClass: 'buying',
    intentionPurpose: 'for_buying',
    predicateLabel: 'visits for buying',
  },
  music: {
    label: 'Music',
    color: INTENTION_HEX.music,
    gradientEnd: '#FF8A65',
    cssClass: 'music',
    intentionPurpose: 'for_music',
    predicateLabel: 'visits for music',
  },
}
...
export const TRUST_ITEMS: {
  ...
  .map(([type, v]) => ({
    type,
    label: v.label.toLowerCase(),
    predicateLabel: v.predicateLabel,
  }))
...
export function getIntentionBadge(
  intention?: string,
...
export function displayLabelToIntentionType(
  label: string,
): IntentionType | null {
  const needle = label.trim().toLowerCase()
  for (const [type, cfg] of Object.entries(INTENTION_CONFIG) as [
    IntentionType,
    IntentionConfigEntry,
  ][]) {
...
export function getSideColor(side: 'support' | 'oppose'): string {
  return side === 'support'
    ? INTENTION_CONFIG.trusted.color
    : INTENTION_CONFIG.distrusted.color
}
...
export function predicateLabelToIntentionType(
  label: string,
): IntentionType | null {
  ...
  switch (trimmed) {
    ...
    case 'distrust':
      return 'distrusted'
    case 'visits for work':
      return 'work'
    case 'visits for learning':
      return 'learning'
    case 'visits for fun':
      return 'fun'
    case 'visits for inspiration':
      return 'inspiration'
    case 'visits for buying':
      return 'buying'
    case 'visits for music':
      return 'music'
    default:
      return null
  }
