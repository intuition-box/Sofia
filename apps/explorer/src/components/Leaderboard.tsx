...
export default function Leaderboard({
  ...
  const sortedPool = useMemo(
    () => (poolData ? sortPool(poolData, poolSortBy) : []),
    [poolData, poolSortBy],
  )
  const sortedTrust = useMemo(
    () => sortTrust(trustRankings, trustSortBy),
    [trustRankings, trustSortBy],
  )
    ...
    <Card className="overflow-hidden lb-card">
      ...
      {isTrust && !loading && (
        <div className="lb-trust-legend">
          <p>
            <strong>Score</strong> — Global trust level computed by EigenTrust
            across the entire attestation graph. Higher means more trusted by
            well-trusted wallets.
          </p>
          <p>
            <strong>Confidence</strong> — How reliable the score is, based on
            the volume and diversity of attestations received.
          </p>
          <p>
            <strong>Paths</strong> — Number of distinct trust paths leading to
            this wallet. More paths = more robust reputation.
          </p>
        </div>
      )}
    </Card>
