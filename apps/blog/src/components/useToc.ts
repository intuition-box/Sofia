...
/**
 * useToc — builds the article table of contents from the *rendered*
 * MDX DOM (real posts are compiled MDX, not a body[] array, so the
 * TOC can't be derived statically). rehype-slug puts an id on every
 * h2; an IntersectionObserver tracks the section in view.
 *
 * `key` should change per post (the slug) so it re-scans on navigation.
 */
