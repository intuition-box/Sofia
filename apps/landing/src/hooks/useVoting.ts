...
export function useVoting() {
            ...
            params: [
              {
                ...
                blockExplorerUrls: [
                  intuitionMainnet.blockExplorers.default.url,
                ],
              },
            ],
          })
   ...
   */
  const ensureProxyApproval = useCallback(
    async (
      walletClient: any,
      publicClient: any,
      account: `0x${string}`,
    ): Promise<void> => {
      const isApproved = await BlockchainService.checkProxyApproval(
        publicClient,
        account,
      )
      ...
      if (!isApproved) {
        ...
        const approvalHash = await BlockchainService.requestProxyApproval(
          walletClient,
          account,
        )
  ...
  const depositFor = useCallback(
    async (tripleId: `0x${string}`): Promise<string> => {
      try {
        ...
        const totalCost = await BlockchainService.getTotalDepositCost(
          publicClient,
          STAKE_AMOUNT,
        )
   ...
   */
  const depositAgainst = useCallback(
    async (tripleId: `0x${string}`): Promise<string> => {
      try {
        const { walletClient, publicClient, account } = await getClients()
        ...
        const counterTripleId = await BlockchainService.getCounterTripleId(
          publicClient,
          tripleId,
        )
