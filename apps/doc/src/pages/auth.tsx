...
const sendFirstClaim = (extensionId: string): Promise<boolean> => {
  return new Promise((resolve) => {
    if (
      ...
      logger.log(
        '[Sofia Auth] Cannot send FIRST_CLAIM: no valid extension ID or chrome API',
      )
      resolve(false)
      return
    }
...
const AuthContent = () => {
  ...
  useEffect(() => {
    if (isConnected && address && !hasSentToExtension) {
      logger.log('[Sofia Auth] Wallet connected, sending to extension:', {
        address,
        walletType,
      })
        ...
        {status === 'success' && address && (
          <>
            ...
            {claimStatus === 'error' && (
              <div className={styles.instructions}>
                <p className={styles.instructionsText}>
                  Could not reach the extension. Make sure Sofia is installed
                  and try again.
                </p>
              </div>
            )}
