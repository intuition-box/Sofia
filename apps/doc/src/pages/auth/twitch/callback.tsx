...
const exchangeCodeForToken = async (code: string) => {
  ...
  if (!response.ok) {
    ...
    logger.error(
      '[Sofia Twitch Callback] Token exchange failed:',
      response.status,
      errorData,
    )
    throw new Error(
...
const sendTokenToExtension = (
  ...
  if (
    ...
    try {
      chrome.runtime.sendMessage(extensionId, message, (response) => {
        logger.log('[Sofia Twitch Callback] Extension response:', response)
      })
    } catch (e) {
      logger.log('[Sofia Twitch Callback] Failed to send to extension:', e)
    }
  }
  ...
  if (window.opener) {
    ...
    logger.log('[Sofia Twitch Callback] Sent postMessage to opener')
  }
  ...
  try {
    ...
    logger.log('[Sofia Twitch Callback] Stored in localStorage')
  } catch (e) {
    logger.log('[Sofia Twitch Callback] LocalStorage not available')
  }
}
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        logger.log('[Sofia Twitch Callback] Token received successfully')
        ...
        logger.error('[Sofia Twitch Callback] Error:', error)
        setStatus('error')
        setErrorMessage(
