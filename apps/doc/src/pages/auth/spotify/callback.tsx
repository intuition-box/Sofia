...
const exchangeCodeForToken = async (code: string) => {
  ...
  if (!response.ok) {
    ...
    logger.error(
      '[Sofia Spotify Callback] Token exchange failed:',
      response.status,
      errorData,
    )
    throw new Error(
...
const sendTokenToExtension = (
  ...
  if (
    ...
    try {
      chrome.runtime.sendMessage(extensionId, message, (response) => {
        logger.log('[Sofia Spotify Callback] Extension response:', response)
      })
    } catch (e) {
      logger.log('[Sofia Spotify Callback] Failed to send to extension:', e)
    }
  }
  ...
  if (window.opener) {
    ...
    logger.log('[Sofia Spotify Callback] Sent postMessage to opener')
  }
  ...
  try {
    ...
    logger.log('[Sofia Spotify Callback] Stored in localStorage')
  } catch (e) {
    logger.log('[Sofia Spotify Callback] LocalStorage not available')
  }
}
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        logger.log('[Sofia Spotify Callback] Token received successfully')
        ...
        logger.error('[Sofia Spotify Callback] Error:', error)
        setStatus('error')
        setErrorMessage(
