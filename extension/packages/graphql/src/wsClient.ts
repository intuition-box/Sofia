 ...
 *
 * Managed as a singleton — one WS connection per context (popup, offscreen,
 * service worker), shared across all subscriptions. The connection is lazy:
 * the socket opens on the first subscribe() call.
 *
 * Configure the WS URL via configureWsClient({ wsUrl }) from client.ts.
 */
 ...
 * Called on wallet change, logout, or when the context unmounts.
 */
