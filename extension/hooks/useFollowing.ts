...
import { useGetMyFollowingQuery, GetMyFollowingQuery, useGetAtomDataByLabelsQuery } from '@0xsofia/graphql'
...
export function useFollowing(walletAddress: string | undefined): FollowQueryResult {
  ...
  const fetchFollowing = useCallback(async () => {
    ...
    try {
      ...
      const response = await useGetMyFollowingQuery.fetcher({
        subjectId: SUBJECT_IDS.I,
        predicateId: PREDICATE_IDS.FOLLOW,
        walletAddress: checksumAddress
      })() as GetMyFollowingQuery
      ...
      const triplesWithPositions = response.triples.filter(
        (triple) => triple.term?.vaults?.length > 0 && triple.term.vaults[0].positions.length > 0
      )
      ...
      let followAccounts: FollowAccountVM[] = triplesWithPositions.map((triple) => {
        ...
        const trustAmountWei = vault.positions.reduce((sum, pos) => {
