...
export function createGaiaNetProvider(config: GaiaNetConfig = {}) {
  const nodeUrl = normalizeUrl(
    config.nodeUrl ||
      process.env.GAIANET_NODE_URL ||
      'https://llama.us.gaianet.network',
  )
 ...
 */
export const GAIANET_TEXT_MODEL_SMALL =
  process.env.GAIANET_TEXT_MODEL_SMALL || GAIANET_DEFAULT_MODEL

/**
 * Large model for text generation
 */
export const GAIANET_TEXT_MODEL_LARGE =
  process.env.GAIANET_TEXT_MODEL_LARGE || GAIANET_DEFAULT_MODEL

/**
 * Embedding model name
 */
export const GAIANET_EMBEDDING_MODEL =
  process.env.GAIANET_EMBEDDING_MODEL || 'Nomic-embed-text-v1.5'
