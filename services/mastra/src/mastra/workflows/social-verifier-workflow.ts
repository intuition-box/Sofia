...
const inputSchema = z.object({
  walletAddress: z
    .string()
    .describe('User wallet address to receive the attestation'),
  tokens: z.object({
...
const executeSocialVerifier = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    console.log(
      `[SocialVerifierWorkflow] Starting verification for ${walletAddress}`,
    )
    ...
    console.log(
      `[SocialVerifierWorkflow] Verified ${verifiedCount}/5 platforms:`,
      verified,
    )
    ...
    console.log(
      `[SocialVerifierWorkflow] All tokens verified! Creating triple on-chain...`,
    )
    ...
    try {
      ...
      console.log(
        `[SocialVerifierWorkflow] User atom exists: ${userAtomExists}`,
      )
      ...
      console.log(
        `[SocialVerifierWorkflow] minDeposit from contract: ${minDeposit}`,
      )
      ...
      if (!userAtomExists) {
        ...
        console.log(
          `[SocialVerifierWorkflow] Atom cost: ${atomCost}, deposit: ${depositAmount}, total: ${atomTotalCost}`,
        )
        ...
        console.log(
          `[SocialVerifierWorkflow] Atom created in block ${atomReceipt.blockNumber}`,
        )
        atomCreated = true
      }
      ...
      console.log(
        `[SocialVerifierWorkflow] Predicate (SOCIALS_PLATFORM) exists: ${predicateExists}`,
      )
      console.log(
        `[SocialVerifierWorkflow] Object (VERIFIED) exists: ${objectExists}`,
      )
      ...
      if (tripleExists) {
        console.log(
          `[SocialVerifierWorkflow] Triple already exists: ${tripleId}`,
        )
        return {
          success: true,
          verified,
      ...
      console.log(
        `[SocialVerifierWorkflow] Bot balance: ${botBalance} wei (${Number(botBalance) / 1e18} ETH)`,
      )
      ...
      console.log(
        `[SocialVerifierWorkflow] Triple cost: ${tripleCost}, extra deposit: ${extraDeposit}, total: ${tripleTotalCost}`,
      )
      console.log(
        `[SocialVerifierWorkflow] Bot has enough balance: ${botBalance >= tripleTotalCost}`,
      )
      ...
      console.log(
        `[SocialVerifierWorkflow] Subject isAtom: ${subjectIsAtom}, Predicate isAtom: ${predicateIsAtom}, Object isAtom: ${objectIsAtom}`,
      )
      ...
      console.log(
        `[SocialVerifierWorkflow] Encoded calldata: ${tripleCallData.slice(0, 100)}...`,
      )
      ...
      try {
        ...
        console.error(
          `[SocialVerifierWorkflow] Gas estimation failed:`,
          estimateError,
        )
      ...
      console.log(
        `[SocialVerifierWorkflow] TX receipt status: ${receipt.status}`,
      )
      console.log(`[SocialVerifierWorkflow] Gas used: ${receipt.gasUsed}`)
      console.log(
        `[SocialVerifierWorkflow] Block number: ${receipt.blockNumber}`,
      )

      if (receipt.status !== 'success') {
        // Try to get more details about the failure
        console.error(
          `[SocialVerifierWorkflow] TX failed! Receipt:`,
          JSON.stringify(
            receipt,
        ...
        if (tripleExistsNow) {
          console.log(
            `[SocialVerifierWorkflow] Triple was created by someone else! Returning success.`,
          )
          return {
            success: true,
            verified,
      ...
      console.log(
        `[SocialVerifierWorkflow] Triple created in block ${receipt.blockNumber}`,
      )
