...
const parseBrowsingData = createStep({
  ...
  inputSchema: z.object({
    rawInput: z
      .string()
      .describe(
        'Raw text input containing URL, title, description, attention score, and visits',
      ),
  }),
  ...
  execute: async ({ inputData }) => {
    ...
    if (!urlMatch || !titleMatch || !attentionMatch || !visitsMatch) {
      throw new Error(
        'Could not parse required fields from input. Expected format: URL: ..., Title: ..., Attention Score: ..., Visits: ...',
      )
    }
...
const transformToTriplet = createStep({
  ...
  description:
    'Transforms browsing data into semantic triplet using SofIA agent',
