...
function formatAtomShares(atom: any): any {
  ...
  if (formatted.term) {
    if (formatted.term.total_market_cap) {
      formatted.term.total_market_cap = formatShares(
        formatted.term.total_market_cap,
      )
    }
  ...
  if (formatted.term?.vaults) {
    formatted.term.vaults = formatted.term.vaults.map((vault: any) => ({
      ...
      market_cap: vault.market_cap
        ? formatShares(vault.market_cap)
        : vault.market_cap,
  ...
  if (formatted.as_subject_triples) {
    formatted.as_subject_triples = formatted.as_subject_triples.map(
      (triple: any) => ({
        ...triple,
        term: triple.term?.vaults
          ? {
              ...triple.term,
              total_market_cap: triple.term.total_market_cap
                ? formatShares(triple.term.total_market_cap)
                : triple.term.total_market_cap,
        ...
        counter_term: triple.counter_term?.vaults
          ? {
              ...triple.counter_term,
              total_market_cap: triple.counter_term.total_market_cap
                ? formatShares(triple.counter_term.total_market_cap)
                : triple.counter_term.total_market_cap,
    ...
    for (const query of args.queries) {
      ...
      if (query.match(/^0x[a-fA-F0-9]{40}$/)) {
        try {
          ...
          `
          const accountResult = (await client.request(accountQuery, {
            address: query.toLowerCase(),
          })) as { account: { label?: string } | null }
          if (
            accountResult.account?.label &&
            accountResult.account.label !== query
          ) {
            expandedQueries.push(accountResult.account.label)
            console.log(
              `Found ENS name for address ${query}: ${accountResult.account.label}`,
            )
          }
        } catch (error) {
      ...
      if (atoms?.length > 0) {
        ...
        atoms.forEach((atom, i) => {
          ...
          if (atom.term?.vaults?.[0]) {
            console.log('- Position count:', atom.term.vaults[0].position_count)
          }
        })
      }
