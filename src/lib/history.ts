import type { NavigationEntry, HistoryData, HistoryFilter } from '../types'
...
export class ChromeHistoryManager {
  ...
  async captureVisit(
    ...
    try {
      ...
      if (!url.startsWith('http') || url.startsWith('chrome://') || url.startsWith('moz-extension://')) {
        return null
      }
  ...
  async filterHistory(
    ...
    if (filter.domain) {
      ...
      filtered = filtered.filter(entry => 
        entry.domain.toLowerCase().includes(domainQuery)
      )
    }
    ...
    if (filter.minDuration) {
      filtered = filtered.filter(entry => 
        entry.visitDuration && entry.visitDuration >= filter.minDuration!
      )
    }
  ...
  private isSensitiveDomain(domain: string): boolean {
    return this.sensitiveDomainsPatterns.some(pattern => 
  ...
  private async categorizeUrl(url: string): Promise<string> {
    ...
    // Catégories sociales
    if (/facebook|instagram|twitter|linkedin|tiktok|snapchat|discord|telegram|whatsapp|reddit/.test(domain)) {
      return 'social'
    }
    ...
    try {
      ...
      const exportData = {
        metadata: {
          ...
          dateRange: {
            oldest: data.entries.length > 0 ? new Date(Math.min(...data.entries.map(e => e.timestamp))).toISOString() : null,
            newest: data.entries.length > 0 ? new Date(Math.max(...data.entries.map(e => e.timestamp))).toISOString() : null
          }
        },
        ...
        entries: data.entries.map(entry => ({
          ...
          visitDurationFormatted: entry.visitDuration ? this.formatDuration(entry.visitDuration) : null
        }))
      }
      ...
      console.log(`Exported ${data.entries.length} entries to JSON (${Math.round(jsonString.length / 1024)}KB)`)
      return jsonString
    } catch (error) {
