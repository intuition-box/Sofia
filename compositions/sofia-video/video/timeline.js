...
const _f6Dx = (_f6WinRect.left + _f6WinRect.width/2) - (_f6C1Rect.left + _f6C1Rect.width/2);
const _f6Dy = (_f6WinRect.top  + _f6WinRect.height/2) - (_f6C1Rect.top  + _f6C1Rect.height/2);
...
const F1_TYPE_START = 1.30;
const F1_TYPE_DUR = 1.80;
const F1_CHAR_DUR = F1_TYPE_DUR / QUERY.length;
for (let i = 0; i <= QUERY.length; i++) {
  tl.set('#f1-text', { textContent: QUERY.slice(0, i) }, F1_TYPE_START + i * F1_CHAR_DUR);
}

// Subtle stage push-in spans the whole pre-zoom window.
tl.to('#f1-stage', { scale: 1, duration: F1_TYPE_START + 0.2, ease: 'power1.out' }, 0.10);

// Cursor blink ladder while the bar is empty + during typing.
for (let i = 0; i < 7; i++) {
  tl.set('#f1-cursor', { opacity: 1 }, 0.20 + i * 0.5);
  tl.set('#f1-cursor', { opacity: 0 }, 0.20 + i * 0.5 + 0.25);
}
...
tl.to('.f1-bar',
  { scale: 1.40, y: -120, duration: F1_TYPE_DUR, ease: 'power3.inOut' },
  F1_TYPE_START);

// Google logo fades out at frame 70 (2.333s @ 30fps).
tl.to('.f1-google',
  { opacity: 0, scale: 0.94, filter: 'blur(8px)', duration: 0.40, ease: 'power2.inOut' },
  70 / 30);

// Hold the zoomed-in bar before F2 takes over.
tl.to('.f1-bar',
  { scale: 1.42, duration: 0.50, ease: 'sine.inOut' },
...
CW_TARGETS.forEach(([x, y, r], i) => {
  tl.to(`#cw-${i}`,
    { x, y, rotation: r, scale: 1, opacity: 1, duration: 0.55, ease: 'back.out(1.3)' },
    F2 + i * 0.045);
});
...
tl.to('#f2-logo',
  { opacity: 1, scale: 2.4, y: 0, filter: 'blur(0px)', duration: 0.55, ease: 'expo.out' },
  F2 + 1.40);
// Stage 2 — outlines draw via strokeDashoffset → 0 (ui-3d-reveal style).
tl.to('#f2-logo .f2-logo-path',
  { strokeDashoffset: 0, duration: 0.70, ease: 'power2.inOut', stagger: 0.08 },
  F2 + 1.50);
...
tl.to('#f2-iris-peach',
  { clipPath: 'circle(150% at 50% 50%)', duration: IRIS_DUR, ease: 'power2.inOut' },
  IRIS_AT);
...
tl.to('#f3-rays',
  ...
  F3 + 0.20);
tl.to('#f3-rays',
  { rotation: 32, duration: 3.0, ease: 'none' },
  F3 + 0.20);

// The single f2-logo SHRINKS as "Introducing Sofia" comes in.
tl.to('#f2-logo',
  { scale: 1, y: -40, duration: 0.70, ease: 'expo.inOut' },
  F3 + 0.40);
tl.to('#f3-intro',
...
tl.to('#f2-sweep',
  ...
  F2 + 3.45);  // 7.95 → 8.5s, F2 ends at 8.5s, F4 takes over right after
...
gsap.set('#f4-noir', { clipPath: 'polygon(100% 0%, 100% 0%, 100% 0%, 100% 0%)' });
tl.to('#f4-noir',
  { clipPath: 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)', duration: 0.55, ease: 'power3.inOut' },
  F4);
...
tl.to('#f4-window', { opacity: 1, x: 0, duration: 0.55, ease: 'power3.out' }, F4 + 0.5);
...
tl.fromTo('#f4-window',
  ...
  { boxShadow: '0 0 0 0 rgba(255, 198, 176, 0.0), 0 0 80px 20px rgba(255, 198, 176, 0.55), 0 32px 72px rgba(122, 58, 30, 0.30)',
    duration: 0.45, ease: 'power2.out' },
  F4 + 0.9);
tl.to('#f4-window',
  { boxShadow: '0 32px 72px rgba(122, 58, 30, 0.22)', duration: 0.55, ease: 'power2.inOut' },
  F4 + 1.35);
...
const GATHER = [
  { cw: '#lcw-0', row: '#f4-url-0', txt: '#f4-url-0-text', url: 'booking.com/thailand-deals',     dx: 910,  dy: 190 },
  { cw: '#lcw-1', row: '#f4-url-1', txt: '#f4-url-1-text', url: 'tripadvisor.com/thailand-hotels', dx: 672,  dy: 190 },
  { cw: '#lcw-2', row: '#f4-url-2', txt: '#f4-url-2-text', url: 'airbnb.com/rooms/chiang-mai',    dx: 434,  dy: 190 },
  { cw: '#lcw-3', row: '#f4-url-3', txt: '#f4-url-3-text', url: 'reddit.com/r/ThailandTourism',   dx: 910,  dy: 42  },
  { cw: '#lcw-4', row: '#f4-url-4', txt: '#f4-url-4-text', url: 'hotels.com/thailand-deals',      dx: 672,  dy: 42  },
  { cw: '#lcw-5', row: '#f4-url-5', txt: '#f4-url-5-text', url: 'expedia.com/Thailand',           dx: 434,  dy: 42  }
];
...
GATHER.forEach((g, i) => {
  ...
  tl.fromTo(g.row,
    ...
    { opacity: 1, x: 0, scale: 1, y: 0, filter: 'blur(0px)', duration: 0.40, ease: 'expo.out' },
    t + 0.32);
  ...
  tl.fromTo('#f4-window',
    ...
    { boxShadow: '0 0 64px 14px rgba(255, 198, 176, 0.55), 0 32px 72px rgba(122, 58, 30, 0.32)',
      duration: 0.14, ease: 'power2.out' },
    t + 0.30);
  tl.to('#f4-window',
    { boxShadow: '0 32px 72px rgba(122, 58, 30, 0.22)', duration: 0.30, ease: 'power2.inOut' },
    t + 0.68);
});
...
tl.to('#f4-validate-btn',
  { opacity: 1, scale: 1, filter: 'blur(0px)', duration: 0.30, ease: 'expo.out' },
  F4 + 2.05);
...
gsap.set('#f4-ptr', { left: _ptrTipX + 105, top: _ptrTipY + 205, opacity: 0, scale: 1 });
tl.to('#f4-ptr', { opacity: 1, duration: 0.15, ease: 'power2.out' }, F4 + 2.20);
tl.to('#f4-ptr',
  { left: _ptrTipX, top: _ptrTipY, duration: 0.45, ease: 'power2.inOut' },
  F4 + 2.25);
// Click pulse
tl.to('#f4-ptr', { scale: 0.82, duration: 0.07, ease: 'power2.in' },  F4 + 2.70);
tl.to('#f4-ptr', { scale: 1,    duration: 0.10, ease: 'power2.out' }, F4 + 2.77);
// Button reacts to the click: snap to clicked state + pulse
tl.fromTo('#f4-validate-btn',
  { scale: 1 },
  { scale: 1.08, duration: 0.08, ease: 'power2.out' },
  F4 + 2.72);
tl.to('#f4-validate-btn',
  { scale: 1, backgroundColor: 'var(--peach)', color: '#000000',
...
tl.to('#f5-window',
  { opacity: 1, scale: 1, y: 0, filter: 'blur(0px)', duration: 0.66, ease: 'back.out(1.3)' },
  F5);
...
tl.to('.f5-card', {
  ...
  duration: 0.66, ease: 'expo.out', stagger: 0.10
}, F5 + 0.25);
...
gsap.set(['#f5-c-0-pill', '#f5-c-1-pill', '#f5-c-2-pill', '#f5-c-3-pill'], { opacity: 0, scale: 0.6 });
gsap.set('#intent-picker', { opacity: 0, x: 0, y: 0, scale: 0.85, pointerEvents: 'none' });
...
function tagCard(t, cardId) {
  ...
  tl.to('#f5-ptr', { left: target.x, top: target.y, duration: 0.22, ease: 'power3.inOut' }, t);
  // 2. Click pulse
  ...
  tl.fromTo(target.pillId,
    { opacity: 0, scale: 0.4, filter: 'blur(4px)' },
    { opacity: 1, scale: 1, filter: 'blur(0px)', duration: 0.22, ease: 'expo.out' },
    t + 0.24);
}

// Four tagging clicks compressed into the 3s F5 window — 0.45s spacing.
tagCard(F5 + 0.40, '#f5-c-1');   // TripAdvisor → TRUSTED
tagCard(F5 + 0.85, '#f5-c-0');   // Booking → TRUSTED
tagCard(F5 + 1.30, '#f5-c-2');   // Airbnb → TRUSTED
...
tl.to('#f5-ptr',
  { left: _pubTipX, top: _pubTipY, duration: 0.50, ease: 'power2.inOut' },
  F5 + 2.30);
// Click pulse
tl.to('#f5-ptr', { scale: 0.82, duration: 0.07, ease: 'power2.in' },  F5 + 2.78);
tl.to('#f5-ptr', { scale: 1,    duration: 0.10, ease: 'power2.out' }, F5 + 2.85);
// Button reacts to the click
tl.fromTo('#f5-publish-btn',
  { scale: 1 },
  { scale: 1.08, duration: 0.08, ease: 'power2.out' },
  F5 + 2.80);
tl.to('#f5-publish-btn',
...
(function spawnConfetti() {
  ...
  const cx = _pubBtn.x;
  const cy = _pubBtn.y;
  const BURST_START = F5 + 2.80; // exact click frame

  for (let i = 0; i < N; i++) {
    const piece = document.createElement('div');
    const shape = SHAPES[Math.floor(rand() * SHAPES.length)];
    const color = COLORS[Math.floor(rand() * COLORS.length)];
    const w = shape === 'rb' ? 5 + rand() * 3 : shape === 'dot' ? 9 + rand() * 4 : 9 + rand() * 5;
    ...
    tl.set(piece, { x: cx, y: cy, scale: 0.5, opacity: 0, rotation: 0 }, BURST_START + stagger);
...
tl.to('#f6-window',
  { opacity: 1, scale: 1, filter: 'blur(0px)', duration: 0.55, ease: 'back.out(1.2)' },
  F6 + 0.05);
gsap.set('.f6-grid .f5-card', { opacity: 0, scale: 0.92, y: 40, filter: 'blur(10px)' });
tl.to('.f6-grid .f5-card', {
  opacity: 1, scale: 1, y: 0, filter: 'blur(0px)',
...
function shakeKeyframes(amp, duration) {
  ...
  const pattern = [1, -0.92, 0.82, -0.71, 0.88, -0.65, 0.55, -0.78, 0.95, -0.5, 0.6, -0.4];
  const kf = [];
...
VOTE_SEQUENCE.forEach((v, i) => {
  ...
  const nextT = (i + 1 < TOTAL_VOTES)
    ? VOTE_SEQUENCE[i + 1].t
    : CAPTION_OUT - 0.20;
  const dur = Math.max(0.10, nextT - v.t);
...
tl.to('#f6-caption', { opacity: 0, duration: 0.35, ease: 'power2.in' }, CAPTION_OUT - 0.35);
...
VOTE_SEQUENCE.forEach((v) => {
  ...
  tl.fromTo(mavId,
    ...
    { opacity: 1, scale: 1.18, y: 0, rotate: 0, filter: 'blur(0px)',
  ...
  tl.to(v.card,
    { y: 4, duration: 0.10, ease: 'power2.out' },
    v.t + 0.05);
  tl.to(v.card,
    { y: 0, duration: 0.30, ease: 'elastic.out(1, 0.45)' },
    v.t + 0.15);
  ...
  gsap.set(floatEl, { left: '100%', top: '50%', marginLeft: 10, opacity: 0, scale: 0.4, rotate: -8, xPercent: 0, yPercent: -50 });
  // Burst in with overshoot.
  tl.fromTo(floatEl,
    { opacity: 0, scale: 0.4, rotate: -8, y: 0 },
    { opacity: 1, scale: 1.45, rotate: 0, y: -8, duration: 0.22, ease: 'back.out(2.4)' },
    v.t + 0.10);
  // Settle.
  tl.to(floatEl, { scale: 1.15, duration: 0.12, ease: 'power2.out' }, v.t + 0.32);
  // Drift up + fade out.
  tl.to(floatEl,
...
gsap.set(['#f7-srp-1', '#f7-srp-2'], { opacity: 0, scale: 0.92, y: 40, filter: 'blur(10px)' });
...
tl.set('#f6-social', { zIndex: 50 }, F7 - 1.20);
tl.set('#f6-c-1', { zIndex: 60, position: 'relative' }, F7 - 1.20);
...
tl.to('.f7-results', { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.66, ease: 'expo.out' }, F7 + 0.00);
tl.to('.f7-section-eyebrow', { opacity: 1, x: 0, filter: 'blur(0px)', duration: 0.46, ease: 'expo.out' }, F7 + 0.10);
tl.to('.f7-section-logo', {
  rotation: 0, opacity: 1,
  duration: 0.6, ease: 'expo.out'
}, F7 + 0.20);
tl.to(['#f7-srp-1', '#f7-srp-2'], {
  opacity: 1, scale: 1, y: 0, filter: 'blur(0px)',
  duration: 0.66, ease: 'expo.out', stagger: 0.14
}, F7 + 1.40);
tl.to('#f7-vanilla', {
...
tl.to('#f7-srp-snake-wrap', {
  opacity: 1, duration: 0.25, ease: 'power2.out'
}, F7 + 0.85);
tl.to('#f7-srp-snake-ring', {
  '--snake-angle': '720deg',
  duration: 1.85,
  ease: 'none'
}, F7 + 0.85);
...
tl.to('#f7-srp-0', {
  scale: 1.06,
  boxShadow: '0 0 0 3px var(--peach), 0 24px 60px -18px rgba(255, 198, 176, 0.55)',
  duration: 0.20,
  ease: 'back.out(2)',
  transformOrigin: '50% 50%'
}, F7 + 1.30);
...
tl.to('#f7-like-cursor',
  ...
  F7 + 2.05);
// 3. Click pulse
tl.to('#f7-like-cursor', { scale: 0.82, duration: 0.06, ease: 'power2.in' },  F7 + 2.70);
tl.to('#f7-like-cursor', { scale: 1,    duration: 0.10, ease: 'power2.out' }, F7 + 2.78);

// 4. Like button reacts to the click — scale pop + peach glow ramp.
tl.to('#f7-thumb', {
  keyframes: [
    { scale: 0.88, duration: 0.06 },
    { scale: 1.20, duration: 0.14 },
...
tl.to('#f7-srp-0', {
  keyframes: [
    ...
    { scale: 1.04, y: -6, rotation: -0.8, duration: 0.10 },
    { scale: 1.01, y: 0,  rotation:  0.6, duration: 0.10 },
    { scale: 1,    y: 0,  rotation:  0,   duration: 0.10 }
  ],
  ease: 'none',
  transformOrigin: '50% 50%'
}, F7 + 2.74);
...
tl.fromTo('#f8-mark',
  ...
  { filter: 'brightness(0) invert(1) drop-shadow(0 0 32px rgba(255,198,176,0.55))',
...
tl.to('#alice-trust-notif',
  { x: 0, opacity: 1, filter: 'blur(0px)', duration: 0.55, ease: 'expo.out' },
  NOTIF_IN);
tl.fromTo('#alice-trust-notif',
  { scale: 1 },
  { scale: 1.015, duration: 0.10, ease: 'power2.out', yoyo: true, repeat: 1 },
  NOTIF_IN + 0.55);
tl.to('#alice-trust-notif',
  { x: 460, opacity: 0, filter: 'blur(6px)', duration: 0.40, ease: 'power3.in' },
  NOTIF_OUT);
...
if (typeof HyperShader !== 'undefined' && HyperShader.init) {
  HyperShader.init({
    ...
    transitions: [
      { time: 2.5,  shader: 'gravitational-lens', duration: 0.6, ease: 'power2.inOut' }, // search → chaos
      // F2 → reveal iris is a CSS clip-path on #f2-iris-peach (no shader).
      { time: 11.1, shader: 'cinematic-zoom',     duration: 0.6, ease: 'power2.inOut' }, // lasso → validate
